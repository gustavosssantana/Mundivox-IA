  ...
  return (
    <div className={cn("fixed bottom-4 left-0 right-0 px-4 pointer-events-none z-10", hasAnimated && "composer-intro")}>
      <div className="relative max-w-2xl mx-auto pointer-events-auto">
        <div
          className={cn(
            "flex flex-col gap-3 p-4 bg-white border-blue-100 transition-all duration-200 border border-blue-100 overflow-hidden relative rounded-3xl",
            "focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-100",
          )}
          style={{
            boxShadow:
              "rgba(37, 99, 235, 0.08) 0px 0px 0px 1px, rgba(37, 99, 235, 0.06) 0px 1px 1px -0.5px, rgba(37, 99, 235, 0.06) 0px 3px 3px -1.5px, rgba(37, 99, 235, 0.06) 0px 6px 6px -3px, rgba(37, 99, 235, 0.06) 0px 12px 12px -6px, rgba(37, 99, 235, 0.06) 0px 24px 24px -12px",
          }}
        >
          <div className="flex gap-2 items-center">
            {uploadedImage && (
              <div className={cn("relative shrink-0", showImageBounce && "image-bounce")}>
                ...
                <button
                  onClick={removeImage}
                  className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center transition-colors"
            ...
            <textarea
              ...
              placeholder={isRecording ? "Ouvindo..." : "Digite uma mensagem... (Shift+Enter para nova linha)"}
              disabled={isStreaming || disabled}
              rows={1}
              className={cn(
                "flex-1 resize-none bg-transparent px-2 py-1.5 text-sm text-slate-800 placeholder:text-slate-400",
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={isStreaming || disabled}
              size="icon"
              className="h-9 w-9 shrink-0 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-full"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="bg-blue-50" asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  disabled={isStreaming || disabled}
            ...
            <span className="text-xs text-slate-500">{currentModel.name}</span>
          </div>
        </div>
      </div>
    </div>
