    ...
    >
      ...
      {messages.length === 0 && !error && !isStreaming && (
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <p className={`text-xl font-semibold text-slate-700 ${hasAnimated ? "text-blur-intro" : ""}`}>
            Olá, eu sou a Mundi
          </p>
          <p className={`text-sm mt-2 text-slate-400 max-w-sm ${hasAnimated ? "text-blur-intro-delay" : ""}`}>
            Sou sua assistente inteligente. Como posso ajudar você hoje?
          </p>
        </div>
      )}
